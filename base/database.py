...
class xref(object):
    ...
    def add_code(cls, ea, target, **reftype):
        ...
        near = seg.bounds(ea) == seg.bounds(target)
...
class set(object):
    ...
    def string(cls, ea, width, terminal, encoding):
        ...
        _, bottom = internal.interface.address.bounds()
        Fwithin_width = utils.fcompose(functools.partial(operator.sub, bottom), functools.partial(functools.partial, operator.gt))
        Finitialized = utils.fcompose(functools.partial(type.flags, mask=idaapi.FF_IVL), functools.partial(operator.eq, idaapi.FF_IVL))

        # Figure out our terminal characters that we'll use for the string length.
        iterable = itertools.chain(bytearray(itertools.islice(itertools.chain(terminal or b'', default * width), width)))
        Fis_terminator = utils.fcompose(functools.partial(read, size=width), functools.partial(operator.eq, builtins.bytes(iterable)))
        ...
        takewhile = utils.fcompose(utils.fmap(Fwithin_width(width), Finitialized, utils.fnot(Fis_terminator)), all)
    ...
    def string(cls, bounds, width, length, encoding):
        ...
        if interface.string.codec(width, encoding) is None:
            raise E.UnsupportedCapability("{:s}.string({:s}, {:d}, {:d}, {!r}) : The requested string encoding ({:s}) is unavailable.".format('.'.join([__name__, cls.__name__]), bounds, width, length, encoding, utils.string.escape(encoding, '"')))

        # Grab its index and then recurse with the correct encoding index.
        index = interface.string.encoding(encoding)
        if index < 0:
            raise E.ItemNotFoundError("{:s}.string({:s}, {:d}, {:d}, {!r}) : The requested string encoding ({:s}) could not be found in the database.".format('.'.join([__name__, cls.__name__]), bounds, width, length, encoding, utils.string.escape(encoding, '"')))
    ...
    def string(cls, bounds, width, length, encoding):
        ...
        # length prefix size fits within the suggestd boundaries so that the user knows.
        if size < length:
        ...
        if Fcreate_string(ea, size, res):
            return get.string(ea, width, length, encoding)
...
class get(object):
    ...
    def string(cls, bounds, **strtype):
        ...
        strtypeinfo, is_string = (default, False) if res in {idaapi.BADADDR, 0xffffffff} else (res, True)
        ...
        if layout > distance:
            logging.warning("{:s}.string({:s}{:s}) : Attempting to apply a string with a prefix length ({:d}) that is larger than the given boundaries ({:s}).".format('.'.join([__name__, cls.__name__]), bounds, u", {!s}".format(utils.string.kwargs(strtype)) if strtype else '', layout, bounds))
    ...
    def string(cls, bounds, width, encoding):
        ...
        # If the codec doesn't exist, then try and add it to the database.
        if interface.string.codec(width, encoding) is None:
            raise E.UnsupportedCapability("{:s}.string({:s}, {:d}, {!r}) : The requested string encoding ({:s}) is unavailable.".format('.'.join([__name__, cls.__name__]), bounds, width, encoding, utils.string.escape(encoding, '"')))

        # Grab its index and then recurse with the correct encoding index.
        index = interface.string.encoding(encoding)
        if index < 0:
            raise E.ItemNotFoundError("{:s}.string({:s}, {:d}, {!r}) : The requested string encoding ({:s}) could not be found in the database.".format('.'.join([__name__, cls.__name__]), bounds, width, encoding, utils.string.escape(encoding, '"')))
    ...
    def string(cls, bounds, width, encoding):
        ...
        if extra:
            logging.warning("{:s}.string({:s}, {:d}, {:d}) : Adjusting the requested boundaries ({:s}) as the desired character width ({:d}) will result in {:d} extra byte{:s}.".format('.'.join([__name__, cls.__name__]), bounds, width, encoding, bounds, width, extra, '' if extra == 1 else 's'))
        ...
        Fdecode = utils.fidentity if codec is None else functools.partial(codec.decode, errors='replace')

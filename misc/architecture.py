...
This module contains class definitions that are used to track the
processors that are available and currently supported by the plugin.
The contents of this module are not intended to be available to the user
and is instead staged by the loader so that the plugin can determine
the processor that is currently being used and switch to the correct
architecture if one has been registered and is available.
"""
...
# that Hex-Rays isn't available for us to actually use.
    ...
    logging.debug("{:s} : Ignoring the Hex-Rays (decompiler) microcode architecture due to a missing \"{:s}\" module.".format(__name__, 'ida_hexrays'))
...
# architecture in order to support the micro-registers from Hex-Rays.
    ...
    class uarchitecture_t(architecture_t):
        ...
        def by_rlist(self, bitset):
            ...
            for _, points in itertools.groupby(enumerate(byteset), utils.funpack(operator.sub)):
                ...
                logging.debug("bitset {:+d} : found {:d} contiguous points in rlist: {!s}".format(offset, len(points), points))

                run = points[:]
                while run:
                    result = self.by_partial(run[0], len(run))
                    size = result.size
                    logging.debug("bitset {:+d} : decoded {:d} run as {!s} : {!r} : left {!r}".format(offset, size, result, run[:size], run[size:]))
                    yield result
        ...
        def by_partial(self, index, size):
            ...
            if any(size == maximum for size in candidates):
                ...
                except (exceptions.RegisterNotFoundError, KeyError):
            ...
            if promotions and result.size < offset + maximum:
                ...
                except exceptions.RegisterNotFoundError:
                    return result
        ...
        def __init__(self, architecture, **cache):
            ...
            for reg, midx in mregindex.items():
                logging.debug("seeded index for register {!r} with uregister {:d}".format(reg, midx))
            ...
            for idx, name in enumerate(idaapi.ph.regnames):
                ...
                if 0 > midx:
                    logging.debug("skipping register {:d} during collection : {!r}".format(idx, reg))
                else:
                    mregindex[reg] = midx
                continue
            ...
            for root in roots:
                ...
                for hidx in used:
                    for (position, ptype), reg in matrix[hidx].items():
                        ...
                        parent = results[reg.__parent__]
                        if midx < 0:
                            logging.debug("skipping register {!r} ({!s}) due to there being no corresponding uregister ({:d})".format(reg, ptype, midx))
    ...
    logging.debug("{:s} : Successfully defined Hex-Rays (decompiler) microcode architecture.".format(__name__))
...
class module(object):
    ...
    def newprc(self, id):
        ...
        if result is None:
            cls = self.__class__
            logging.warning("{:s}.newprc({:d}) : Unsupported processor type {:d} was specified. Tools that use the instruction module might not work properly.".format('.'.join([__name__, cls.__name__]), id, plfm))
            return
